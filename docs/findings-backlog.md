...
> **UPDATE (2026-07-01)**: Full Ghidra decompilation completed (141,343 functions, 6.7M lines). Many findings below have been resolved. See `research/32bit_ghidra_findings.md` for the current state. Key corrections:
> - Graphics API offset is **0x160** (32-bit), NOT 0x1d8 (which is 64-bit)
> - Gate CLEAR found at `0x0173ce00` (`RecvMsgAppStatus`)
> - Controller constructor map fully traced (CHIDIOThread 0xBB78 bytes, Master 0xC34 bytes, per-slot 0xDC bytes)
> - SET_SETTINGS confirmed fire-and-forget (no response read)
> - BLE/USB transport field at `controller+0xbc` (1=USB, 2=BLE)
> - Init chain stall root cause: CCCD subscription timing gap
> - **Firmware jump table resolved** — TBH at `0x383d2`, 144 entries. 0xf2 is a minimal ACK (6 bytes, no payload), NOT a capability response.
> - **0x80 is motor output**, NOT 0x8F. 0x8F is the host-side sub-command router.
> - **Firmware binary is 33.4% of flash** — descriptor structures at 0x59b10-0x5a332 are beyond the dump. Full flash dump needed.
...
**0x8F gate hypothesis (VERIFIED, gate CLEAR found 2026-07-01):**
- `[esi+0x17c]` at `0x0123e5fb` gates 0x8F dispatch — **VERIFIED** (audit confirmed instruction)
- Gate SET at `0x0178a140`: `mov byte [esi+0x17c], 1` — **VERIFIED**
- Gate CLEAR at `0x0173ce00` (`RecvMsgAppStatus`): `*(param_2 + 0x17c) = 0` — **VERIFIED** (only ONE function clears it)
- Gate is SET by 5 different functions but cleared only by `RecvMsgAppStatus` after successful controller status loop processing
...
- Root cause: Init chain stalls at `CGetControllerInfoWorkItem::RunFunc` (0x01218840) because CCCD subscription timing gap prevents notifications from reaching `/dev/hidrawN`. **Confidence: Confirmed**
...
**Status: RESOLVED (2026-07-01) — It is graphics API type, at offset 0x160 in 32-bit**

The function at `0x019aec80` (size 29,672) writes graphics API type to `[eax+0x160]`:
- `0x019af423`: `mov dword [eax+0x160], 1` (OpenGL)
- `0x019af4b6`: `mov dword [eax+0x160], 2` (Vulkan)
- `0x019af5be`: `mov dword [eax+0x160], 3` (D3D12A)

The 32-bit dispatcher at `0x01789ea0` reads `[eax+0x160]` and branches on values 1-4. **The 32-bit offset is 0x160, NOT 0x1d8 (which is the 64-bit offset).** The difference is due to pointer size differences (4 vs 8 bytes) across ~30 pointer-sized fields.

#### Finding: Values 3 and 4 Are Never Statically Written to 0x1d8

**Status: RESOLVED (2026-07-01) — Values ARE written to 0x160 in 32-bit**

Values 1, 2, and 3 are written as immediate values to offset 0x160 in `FUN_019aec80`. Value 4 (D3D12B) was not found as a direct immediate write in this function.
