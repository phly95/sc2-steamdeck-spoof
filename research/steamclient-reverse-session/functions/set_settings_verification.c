 ...
 * - 0x014fd614 is NOT `mov al, 0x87` — it's `movzx esi, byte [rax+0x87]`
 *   (reads command byte from struct, not loads immediate)
 * - 0x014fd620 is the same pattern (re-read after conditional store)
 ...
 *
 * 1. Settings are queued as 16-byte entries in an array at [r15+0xc0]
 ...
 *   0x014fd614  →  Actually: movzx esi, byte [rax+0x87]  (READ from struct)
 *   0x014fd620  →  Actually: movzx esi, byte [rax+0x87]  (RE-READ after conditional)
 *   0x014fdf44  →  Actually: movzx esi, byte [rax+0x87]  (READ from struct)
 *   0x014fdf50  →  Actually: movzx esi, byte [rax+0x87]  (RE-READ after conditional)
 *
 ...
 *   0x010d466b  — main state manager entry
 *   0x010d4e14  — send feature report (vtable[0x10])
 *   0x010d4e83  — read back FR 0x00 (vtable[0x130])
 *   0x010d4dc6  — byte comparison loop
 */

/*
 * === STATE MANAGER STRUCTURE (at r15) ===
 *
 ...
 * +0x128   8      Vtable pointer 1 (compared to 0x010cdde0)
 * +0x150   8      Vtable pointer 2 (compared to 0x010cea50)
 * +0x178   4      State field
 * +0x198   4      Feature report ID for get_feature_report
 * +0x1f8   1      Flag byte
 * +0x208   1      PENDING SETTINGS FLAG (controls verify path)
 *
...
/*
 * === SETTINGS ARRAY ENTRIES (16 bytes each at [r15+0xc0]) ===
 *
 * Offset  Size  Description
 ...
 *   0x010d4da0: cmp byte [r15+0x208], 0    ; any pending settings?
 *   0x010d4da8: movzx eax, byte [r15+0xe1]  ; load verify state
 *   0x010d4db0: je 0x10d4fd0                ; if no pending, go to byte check
 *
 * STEP 2: Toggle verify state
 *   0x010d4db6: mov r12d, eax               ; save current state
 *   0x010d4db9: xor r12d, 1                 ; toggle: 0→1, 1→0
 *   0x010d4dbd: cmp r12b, al                ; compare toggled vs current
 *   0x010d4dc0: je 0x10d4fe5                ; if same (impossible), skip
 *
 * STEP 3: SEND (when state toggles to 1)
 *   0x010d4e11: mov rdi, [rax]              ; load HID device
 *   0x010d4e14: call qword [rax+0x10]       ; SEND FEATURE REPORT
 *   0x010d4e17: test r12b, r12b             ; check which state
 *   0x010d4e1a: mov byte [r15+0xe1], r12b   ; store new state
 *   0x010d4e24: jne 0x10d4e6c               ; if state=1, go to verify
 *
 * STEP 4: READ BACK (when state toggles to 0)
 *   0x010d4e75: mov rax, [r13]              ; load protocol vtable
 *   0x010d4e7c: mov esi, [r15+0x198]        ; feature report ID
 *   0x010d4e83: call qword [rax+0x130]      ; GET FEATURE REPORT (FR 0x00)
 *
 * STEP 5: Compute retry count from return value
 *   0x010d4e89: pxor xmm0, xmm0
 *   0x010d4e8d: cvtsi2ss xmm0, eax          ; convert return value to float
 *   0x010d4e91: divss xmm0, [0x00c84180]    ; divide by constant A
 *   0x010d4e99: mulss xmm0, [0x00c820bc]    ; multiply by constant B
 *   0x010d4ea1: cvttss2si edx, xmm0         ; truncate to int
 *   0x010d4ea5: test edx, edx               ; retry count > 0?
 *   0x010d4ea7: jle 0x10d4f01               ; if ≤ 0, skip retry
 *
 * STEP 6: Byte comparison loop
 *   0x010d4fd0: mov rdx, [r15+0xc0]         ; load settings array
 *   0x010d4fd7: movzx r12d, byte [rdx+rbx]  ; read response byte
 *   0x010d4fdc: cmp r12b, al                ; compare with expected
 *   0x010d4fdf: jne 0x10d4dc6               ; MISMATCH → retry
 *
 * STEP 7: Success
 *   0x010d4fe5: xor r12d, r12d              ; r12d = 0 (success)
 *   0x010d4feb: jne 0x10d4e75               ; if more data, loop
 *
 * STEP 8: Clear pending flag
 *   0x010d50ba: test r12b, r12b
 *   0x010d50bd: mov byte [r15+0xe1], r12b    ; store verify result
 *   0x010d50c4: je 0x10d4e2d                ; if 0, loop back
 *
 * STEP 9: Failure/timeout
 *   0x010d5100: mov esi, 1                   ; return error
 *   0x010d5105: jmp 0x10d4e6c                ; exit
 *   0x010d5174: mov qword [r15+0xe8], 0      ; clear state
 *   0x010d517f: jmp 0x10d4c96                ; restart
 */
 ...
 *   Bytes 6-63: padding
 *
 * The verification at 0x010d4dc6-0x010d4fdf checks:
 *
 *   1. The RESPONSE BYTE at [rdx+rbx] matches the EXPECTED VALUE in al
 ...
 * index (rbx) and the expected value (al).
 */

/*
 * === KEY INSIGHT: THE VERIFICATION IS NOT A FULL BUFFER COMPARISON ===
 *
 * The code at 0x010d4dc6-0x010d4fdf is a LOOP that checks individual
 * bytes, not a memcmp of the entire 64-byte buffer. The loop:
 *
 *   1. Loads a response byte from [rdx+rbx]
 ...
 *   0x01f84306: cmp rax, 0x2dc6c0    ; compare elapsed time with 3s
 *   0x01f8430c: cmovg rax, rdx       ; clamp to max
 *
 ...
 *
 * The timer at 0x02a04238 also uses 0x2DC6C0 for a 3-second timeout.
 */
 ...
 *
 *   0x010d4c50: lea rcx, [0x010cdde0]    ; expected vtable type 1
 *   0x010d4c60: cmp rdx, rcx              ; verify
 *   0x010d4c63: jne 0x10d52b4             ; mismatch → error
 *
 *   0x010d4c70: lea rdx, [0x010cdd90]    ; expected vtable type 2
 *   0x010d4c7a: cmp rax, rdx              ; verify
 *   0x010d4c7d: jne 0x10d52c3             ; mismatch → error
 *
 *   0x010d4eb3: lea rdi, [0x010cde90]    ; expected protocol handler
 *   0x010d4ece: cmp rax, rdi              ; verify
 *   0x010d4ed1: jne 0x10d5238             ; mismatch → error
 *
 *   0x010d500b: lea rdx, [0x010cea50]    ; expected vtable type 3
 *   0x010d5019: cmp rax, rdx              ; verify
 *   0x010d501c: jne 0x10d5260             ; mismatch → error
 *
 ...
 *
 * From the string table at 0x02ae1ac8:
 *
 *   0x02ae1ae0: "IControllerVirtualSocket"           (base interface)
 *   0x02ae1af0: "CControllerHidVirtualSocket"        (HID transport)
 *   0x02ae1b08: "CSteamControllerProtocolHandlerV1"  (V1 protocol)
 *   0x02ae1b20: "CSteamControllerProtocolHandlerV2"  (V2 protocol)
 *   0x02ae1b38: "CGenericControllerProtocolHandler"  (generic fallback)
 *
 * The V1 protocol handler at 0x02ae1b08 has vtable at 0x02c06aa8.
 * The state manager verifies the handler against 0x010cde90 and 0x010cdce0.
 */

/*
 * === BINARY REFERENCES ===
 *
 * State manager: 0x010d466b
 * Send feature report: 0x010d4e14 (vtable[0x10])
 * Get feature report: 0x010d4e83 (vtable[0x130])
 * Byte comparison loop: 0x010d4dc6-0x010d4fdf
 * Timeout comparison: 0x01f84306 (0x2DC6C0 = 3s)
 * Timer start: 0x02a04238 (0x2DC6C0 = 3s)
 * Settings init: 0x0112daf0
 * Settings queue: 0x010d5488
 * Vtable type checks: 0x010cdde0, 0x010cdd90, 0x010cde90, 0x010cea50
 * Protocol handler vtable: 0x02c06aa8
 *
 * String references:
 * "toggle_lizard" at 0x00ca6b5d (in URI string)
 * "CSteamControllerProtocolHandlerV1" at 0x02ae1b08
 ...
 *   3. r12d is set to 0 (success) at 0x010d4fe5
 *   4. The verify state byte [r15+0xe1] is cleared
 *   5. The pending flag [r15+0x208] is cleared
 *   6. The state machine moves to the next setting
 *
 * Verification FAILS when:
 *   1. A response byte doesn't match (jne at 0x010d4fdf)
